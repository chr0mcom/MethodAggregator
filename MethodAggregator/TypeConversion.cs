...
namespace MethodAggregator
{
	...
	{
		[NotNull] private static readonly Dictionary<Type, TypeNode> TypeNodeDictionary = new Dictionary<Type, TypeNode>();
		...
		public static Type GetHighestInheritedType([NotNull] Type callerType, [NotNull] IEnumerable<Type> methodTypes)
		{
			...
			if (!TypeNodeDictionary.TryGetValue(callerType, out TypeNode buildTypeNode))
			{
				...
				TypeNodeDictionary.Add(callerType, buildTypeNode);
			}
		...
		[NotNull] private static TypeNode BuildTypeTree(Type type)
		{
		...
		[NotNull] private static List<Type> GetAllBaseTypes([NotNull] Type type)
		{
			if (type == null) throw new ArgumentNullException(nameof(type));
		...
		[NotNull] private static List<Type> GetAllImplementedTypes([NotNull] Type type)
		{
			if (type == null) throw new ArgumentNullException(nameof(type));
		...
		[NotNull] private static IEnumerable<(Type methodType, TypeNode assignableType)> GetAssignableTypes([NotNull] TypeNode typeNode, [NotNull] List<Type> methodTypes)
		{
			if (typeNode == null) throw new ArgumentNullException(nameof(typeNode));
			if (methodTypes == null) throw new ArgumentNullException(nameof(methodTypes));
		...
		[NotNull] private static List<Type> GetCurrentLevelTypes([NotNull] Type rootType)
		{
		...
		private static int GetIntegerMatchScore(Type inputType, Type targetType)
		{
			...
			if (targetType == typeof(short))
			{
				...
				// check for compatibility based on range
				int inputMax = (int) GetMaxValue(inputType);
